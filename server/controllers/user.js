...
async function signUp(req, res) {
  ...
  /*Esta funcion tiene como objetivo hacer que el primer usuario creado sea administrador*/
  await User.findOne().then((userStored) => {
    if (userStored) {
      user.active = false;
    } else {
      user.active = true;
...
function sigUpAdmin(req, res) {
  const user = new User();

  const { name, lastname, role, password, email } = req.body;
  user.name = name;
  user.lastname = lastname;
  user.email = email.toLowerCase();
  user.role = role;
